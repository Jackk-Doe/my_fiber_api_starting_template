...
func SetLoggerMiddleware(app *fiber.App) {
	...
	myLogger := logger.Config{
		Output: os.Stdout,
		Format: `[API] ${cyan}${time} | REQUEST_ID: ${locals:requestid} | ${yellow}${status}${reset} | ${blue}${method}${reset} | ${yellow}${latency}${reset} ` +
			`| IP: ${green}${ip}${reset} | PATH: ${magenta}${path}${reset} | QUERY_PARAM: ${queryParams} ` +
